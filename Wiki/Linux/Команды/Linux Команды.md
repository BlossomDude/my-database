...
`at` - команда для запланирования задачи
...
apt show <package> #Посмотреть информацию о пакете
```
dpkg
```bash
...
dpkg --listfiles <package> #Посмотреть файлы пакете
взлп --search <path/file> # Узнать к какому пакету относится файл 
